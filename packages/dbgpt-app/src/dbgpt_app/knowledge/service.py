...
class KnowledgeService:
    ...
    def async_doc_embedding(self, assembler, chunk_docs, doc):
        ...
        logger.info(
            f"async doc embedding sync, doc:{doc.doc_name}, chunks length is "
            f"{len(chunk_docs)}"
        )
        try:
    ...
    def _build_default_context(self):
        from dbgpt_app.scene.chat_knowledge.v1.prompt import (
            _DEFAULT_TEMPLATE,
            PROMPT_SCENE_DEFINE,
        )
    ...
    def get_space_context(self, space_name):
        ...
        if len(spaces) != 1:
            raise Exception(
                f"have not found {space_name} space or found more than one space "
                f"called {space_name}"
            )
        space = spaces[0]
        if space.context is not None:
    ...
    async def _llm_extract_summary(
        ...
        from dbgpt_app.openapi.api_v1.api_v1 import CHAT_FACTORY
