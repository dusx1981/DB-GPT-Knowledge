...
const Playground: NextPage = () => {
  ...
  useEffect(() => {
    // 直接创建新对话并跳转到聊天页面
    const createNewDialogueAndRedirect = async () => {
      ...
      //     }),
      //   );
      //   router.push(`/chat?scene=${res.chat_mode}&id=${res.conv_uid}${model ? `&model=${model}` : ''}`);
      // }
      router.push(`/chat`);
    };

    createNewDialogueAndRedirect();
  }, [router, model, setCurrentDialogInfo]);
    ...
    >
      ...
      >
        ...
        <div className='flex flex-col items-center justify-center h-full'>
          <Spin size='large' tip={t('redirecting_to_chat') || '正在跳转到聊天...'} />
        </div>
      </ConfigProvider>
    </div>
  );
